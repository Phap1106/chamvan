...
import {
  getProvinces, getDistrictsByProvinceCode, getWardsByDistrictCode,
...
type Address = {
  ...
  street: string; isDefault?: boolean;
};
...
export default function CheckoutPage() {
  ...
  const [addr, setAddr] = useState<Address>({ phone:'', fullName:'', street:'', isDefault:false });

  useEffect(()=> setProvinces(getProvinces()),[]);
  useEffect(() => {
    if (!addr.province){ setDistricts([]); setWards([]); return; }
    setDistricts(getDistrictsByProvinceCode(addr.province.code));
    setAddr(a=>({ ...a, district:undefined, ward:undefined })); setWards([]);
  },[addr.province?.code]);
  useEffect(() => {
    if (!addr.district){ setWards([]); return; }
  ...
  const [companyInvoice, setCompanyInvoice] = useState({ name:'', address:'', taxId:'' });

  /* B3 */
  const [note, setNote] = useState(''); const [noteChips, setNoteChips] = useState<string[]>([]);
  const toggleChip = (v:string)=> setNoteChips(s=> s.includes(v) ? s.filter(x=>x!==v) : [...s,v]);

  const shippingFee = 0; const discount = 0;
  const total = useMemo(()=> subtotal + shippingFee - discount,[subtotal]);

  const openConfirmAddress = (e:React.FormEvent)=> { e.preventDefault(); setShowConfirm(true); };
  const confirmAddress = ()=> { setSavedAddr(addr); setShowConfirm(false); };
  ...
  async function downloadInvoicePDF(){
    ...
    let y=64;
    doc.setFontSize(20); doc.text('CHAMVAN - HOÁ ĐƠN BÁN HÀNG',40,y); y+=28;
    doc.setFontSize(12); doc.text(`Ngày: ${new Date().toLocaleString('vi-VN')}`,40,y); y+=20;
    if(savedAddr){
      const full=[savedAddr.street, savedAddr.ward?.name, savedAddr.district?.name, savedAddr.province?.name].filter(Boolean).join(', ');
      doc.text(`Khách hàng: ${savedAddr.fullName}`,40,y); y+=18;
      doc.text(`SĐT: ${savedAddr.phone}`,40,y); y+=18;
      doc.text(`Địa chỉ: ${full}`,40,y); y+=22;
    }
    ...
    doc.setFontSize(14); doc.text('Chi tiết đơn hàng',40,y); y+=12;
    doc.setLineWidth(.6); doc.line(40,y,555,y); y+=14; doc.setFontSize(12);
    items.forEach((it,i)=>{ doc.text(`${i+1}. ${it.name}${it.color ? ` (${it.color})`:''} x${it.qty} — ${currency(it.price*it.qty)}`,40,y); y+=18; });
    ...
    if(fullNote){ doc.setFontSize(12); doc.text('Ghi chú khách hàng:',40,y); y+=16; doc.text(doc.splitTextToSize(fullNote, 500),40,y); y+=36; }
    doc.setFontSize(12); doc.text('Đại diện cửa hàng',400,y); y+=56; doc.setFontSize(13); doc.text('CHAMVAN (đã ký)',396,y);
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Thanh top như Hanoia */}
      <div className="border-b">
        <div className="mx-auto flex h-[64px] max-w-[1600px] items-center justify-between px-8">
          <Link href="/gio-hang" className="text-[14px] font-semibold hover:underline">TRỞ LẠI GIỎ HÀNG</Link>
          <div className="font-[var(--font-display)] text-[42px] md:text-[50px] tracking-wide">chamvan</div>
          <span />
        </div>
      </div>
          ...
          <section className="cv-card">
            ...
            {!savedAddr ? (
              <form onSubmit={openConfirmAddress} className="grid grid-cols-1 gap-4 p-5">
                <Field label="Số điện thoại *" value={addr.phone} onChange={(v)=>setAddr({...addr, phone:v})} required />
                <Field label="Họ và tên *" value={addr.fullName} onChange={(v)=>setAddr({...addr, fullName:v})} required />

                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                  <Select
                    label="Tỉnh/Thành phố *"
                    value={addr.province?.code ?? ''}
                    onChange={(code)=> setAddr(a=>({...a, province: getProvinces().find(p=>p.code===code)}))}
                  ...
                  <Select
                    ...
                    onChange={(code)=> setAddr(a=>({...a, district: districts.find(d=>d.code===code)}))}
                  ...
                  <Select
                    ...
                    onChange={(code)=> setAddr(a=>({...a, ward: wards.find(w=>w.code===code)}))}
                ...
                <Field label="Địa chỉ cụ thể *" value={addr.street} onChange={(v)=>setAddr({...addr, street:v})} required />
              ...
              <div className="p-5">
                <div className="rounded-[12px] border border-[var(--cv-border)] bg-neutral-50">
                  ...
                  <Row
                    label="Địa chỉ"
                    value={[savedAddr.street, savedAddr.ward?.name, savedAddr.district?.name, savedAddr.province?.name].filter(Boolean).join(', ')}
                    last
                  />
                </div>
                ...
                <div className="mt-4">
                  <RadioLine
                    ...
                    onChange={()=> setSavedAddr(a=> a ? ({...a, isDefault: !a.isDefault}) : a)}
                ...
                <div className="flex items-center gap-3 mt-4">
                  <button className="btn-lg btn-ghost" onClick={()=>{ setSavedAddr(null); setStep(1); }}>CHỈNH SỬA</button>
                  <button className="btn-lg btn-black" onClick={()=> setStep(2)}>TIẾP TỤC MUA HÀNG</button>
                </div>
              </div>
            )}
          </section>
          ...
          {step>=2 && (
            <section className="mt-8 cv-card">
              ...
              <div className="p-5 space-y-4">
                <RadioCard checked={payment==='cod'}    onChange={()=>setPayment('cod')}
                  title="Thanh toán khi nhận hàng (COD)" desc="Kiểm tra hàng rồi thanh toán."/>
                <RadioCard checked={payment==='online'} onChange={()=>setPayment('online')}
                  title="Thanh toán online qua VNPAY/Thẻ"   desc="Quét QR / thẻ ATM nội địa / thẻ quốc tế."/>

                <div className="pt-2">
                  <div className="mb-2 text-[14px] font-extrabold">HÓA ĐƠN</div>
                  <RadioCard checked={invoiceMode==='none'} onChange={()=>setInvoiceMode('none')}  title="Không xuất hóa đơn"/>
                  <RadioCard checked={invoiceMode==='company'} onChange={()=>setInvoiceMode('company')}
                    title="Xuất hóa đơn công ty" desc="Điền thông tin doanh nghiệp bên dưới."/>
                </div>

                {invoiceMode==='company' && (
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <Field label="Tên công ty *" value={companyInvoice.name} onChange={(v)=>setCompanyInvoice(s=>({...s,name:v}))} required/>
                    <Field label="Địa chỉ công ty *" value={companyInvoice.address} onChange={(v)=>setCompanyInvoice(s=>({...s,address:v}))} required/>
                    <Field label="Mã số thuế *" value={companyInvoice.taxId} onChange={(v)=>setCompanyInvoice(s=>({...s,taxId:v}))} required/>
                  </div>
                )}

                <button className="btn-lg btn-black" onClick={()=> setStep(3)}>TIẾP TỤC</button>
              </div>
            </section>
          )}
            ...
            <section className="mt-8 cv-card">
              ...
              <div className="p-5 space-y-4">
                <div className="flex flex-wrap gap-2">
                  {quickNotes.map(q=>(
                    <button key={q} onClick={()=>toggleChip(q)}
                      className={`rounded-full border px-3 py-1.5 text-[13px] ${noteChips.includes(q) ? 'bg-black text-white border-black':'hover:bg-neutral-50'}`}>
                      {q}
                    </button>
                  ))}
                </div>
                ...
                <label className="block text-[14px]">
                  ...
                  <textarea rows={5} value={note} onChange={(e)=>setNote(e.target.value)}
                    className="w-full input-neo" placeholder="Ví dụ: Giao sau 18h, gọi trước khi giao…" />
                </label>

                <button className="btn-lg btn-black" onClick={handlePlaceOrder}>ĐẶT HÀNG</button>
              </div>
            </section>
          )}
        </div>
        ...
        <aside className="cv-card h-max lg:sticky lg:top-[88px]">
          ...
          <div className="border-t px-5 py-5 text-[14px]">
            ...
            <div className="pt-3 mt-3 border-t">
              <RowInline label={<span className="font-extrabold">TẠM TÍNH</span>}
        ...
        <Modal onClose={()=>setShowConfirm(false)}>
          <div className="w-[620px] max-w-[94vw] overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div className="p-6 space-y-3 text-[14px]">
              ...
              <RowFlat label="Địa chỉ"
                value={[addr.street, addr.ward?.name, addr.district?.name, addr.province?.name].filter(Boolean).join(', ') || '—'} />
            </div>
              ...
              {askInvoice ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button className="btn-lg btn-ghost"
                      onClick={()=>{ setOrderPlaced(false); setAskInvoice(false); }}>Không, cảm ơn</button>
                    <button className="btn-lg btn-black" onClick={downloadInvoicePDF}>Tải hoá đơn (PDF)</button>
                  </div>
                </>
              ) : (<div className="py-2 text-center text-[12px] text-neutral-600">Đang chuẩn bị…</div>)}

              <div className="grid grid-cols-2 gap-2 mt-1">
                <Link href="/" className="grid place-items-center btn-lg btn-ghost"
                  onClick={()=> setOrderPlaced(false)}>Về trang chủ</Link>
                <Link href="/tat-ca-san-pham" className="grid place-items-center btn-lg btn-black"
                  onClick={()=> setOrderPlaced(false)}>Tiếp tục mua hàng</Link>
              </div>
            </div>
          </div>
        </Modal>
...
function Select({ label, value, onChange, options, disabled, required }:{
  ...
  return (
    <label className="block text-[14px]">
      ...
      <select value={value} onChange={(e)=>onChange(e.target.value)} disabled={disabled} required={required}
              className="w-full bg-white input-neo">
    ...
    <button type="button" onClick={onChange}
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          {desc && <div className="text-[12px] text-neutral-500">{desc}</div>}
        </div>
      </div>
    </button>
  );
...
function TruckIcon(){
  return (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-neutral-700">
    <path d="M3 7h10v7H3V7Zm10 3h4l3 4h-7v-4Z" stroke="currentColor" strokeWidth="1.5"/><circle cx="7" cy="18" r="1.5" fill="currentColor"/><circle cx="17" cy="18" r="1.5" fill="currentColor"/></svg>);
}
function CheckIcon(){
  return (<svg width="42" height="42" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="white" strokeWidth="2"/><path d="M7 12.5 10.5 16 17 9" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>);
}
