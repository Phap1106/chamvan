...
import { useEffect, useState } from 'react';
import AccountLayout from '../tai-khoan/_components/AccountLayout';
import { Bug, Type, AlignLeft, SendHorizonal, CheckCircle2, X } from 'lucide-react';

// Lấy API base từ ENV (ưu tiên) hoặc fallback local
const API_BASE =
...
export default function ReportBugPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/reports`, {
        method: 'POST',
        credentials: 'include',             // gửi cookie JWT sang BE (khác origin)
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, description: desc, pageUrl }),
      });
      ...
      setErr(e?.message || 'Không gửi được báo cáo. Vui lòng thử lại.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  useEffect(() => {
    ...
    setShowToast(true);
    const hide = setTimeout(() => setShowToast(false), 2500);
    const reset = setTimeout(() => setSent(false), 2700);
    return () => {
      clearTimeout(hide);
      clearTimeout(reset);
    };
  ...
  return (
    <AccountLayout title="Báo cáo lỗi">
      <form onSubmit={submit} className="max-w-3xl">
        ...
        {err && (
          <div className="mb-4 rounded border border-rose-200 bg-rose-50 px-3 py-2 text-rose-700 text-sm">
            {err}
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-[13px] text-zinc-500">
            * Cảm ơn bạn đã giúp cải thiện sản phẩm.
          </span>
        </div>
      </form>
