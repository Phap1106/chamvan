...
//   // tránh nháy/redirect sớm khi hydrate
...
//   // ✅ chỉ redirect KHI đã xác định đang đăng nhập (true)
//   useEffect(() => {
//     if (!mounted) return;
//     if (isLoggedIn === true && !isAdmin) {
//       router.replace('/'); // có login nhưng không đủ quyền
//     }
//     // (không ép redirect khi isLoggedIn === false để tránh đá sớm lúc app chưa kịp lấy phiên)
//     // Nếu muốn buộc login, có thể dùng middleware hoặc thêm else-if ở đây.
...
//       { href: '/admin/users', label: 'Quản lý user' },            // <-- thêm mục Users
...
//           <div className="sticky top-[84px] rounded-lg border bg-white shadow-sm">
...
// function MobileMenu({ items }: { items: Array<{ href: string; label: string }> }) {
//   const pathname = usePathname();
...
//           <div className="fixed inset-y-0 left-0 z-50 w-[78%] max-w-[320px] bg-white shadow-2xl md:hidden">
//             <div className="flex items-center justify-between px-4 py-3 border-b">
//               <div className="text-sm font-semibold">Quản trị hệ thống</div>
//               <button className="w-8 h-8 border rounded" onClick={() => setOpen(false)} aria-label="Đóng">
//                 ✕
//               </button>
//             </div>
//             <nav className="p-2 text-sm">
