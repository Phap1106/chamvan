...
// /* ================== Fixed slugs ================== */
// const L1_SLUGS = ['hang-moi', 'qua-tang', 'trang-tri-nha'] as const;
// const L2_SLUGS = ['phong-tho', 'phong-khach', 'phong-thuy', 'trung-bay'] as const;
...
//       }));
//       setAllCats(items);
//       setTops(items.filter((c) => (L1_SLUGS as readonly string[]).includes(c.slug)));
//       setSubs(items.filter((c) => (L2_SLUGS as readonly string[]).includes(c.slug)));
//     } catch {
//       setAllCats([]);
//       setTops([]);
//       setSubs([]);
...
//   /* ======= Helpers ======= */
//   const selectedTop = useMemo(() => tops.find((t) => t.id === lv1) ?? null, [tops, lv1]);
//   const isTrangTriNha = selectedTop?.slug === 'trang-tri-nha';
//   const selectedCount = (lv1 ? 1 : 0) + (isTrangTriNha ? lv2.length : 0);
//   const toggleLv2 = (id: number) =>
//     setLv2((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
...
//     setGallery((p.images || []).map((u) => ({ url: u, kind: 'url' as const })));
//     const catSlugs = (p.categories || []).map((c) => c.slug);
//     const top = tops.find((t) => catSlugs.includes(t.slug));
//     setLv1(top ? top.id : null);
//     setLv2(top?.slug === 'trang-tri-nha' ? subs.filter((s) => catSlugs.includes(s.slug)).map((x) => x.id) : []);
//     setTab('url');
//   }
...
//     if (!allCats.length || !tops.length) return notify('error', 'Không lấy được danh mục.'), setErr('Không lấy được danh mục.');
//     if (!p.name.trim()) return notify('error', 'Tên sản phẩm không được trống'), setErr('Tên sản phẩm không được trống');
//     if (!lv1 && !isTrangTriNha) return notify('error', 'Vui lòng chọn danh mục bậc 1'), setErr('Vui lòng chọn danh mục bậc 1');
//     if (isTrangTriNha && lv2.length === 0)
//       return notify('error', 'Chọn ít nhất 1 danh mục bậc 2'), setErr('Chọn ít nhất 1 danh mục bậc 2');
...
//     const categories = isTrangTriNha ? lv2 : lv1 ? [lv1] : [];
...
//     status: (p.status === 'closed' ? 'closed' : 'open'), // mới
//     };
...
//   /* ================== Render ================== */
//   return (
//     <div className="p-1 space-y-4 bg-slate-50 md:p-2">
//       {/* Toasts */}
...
//             key={t.id}
//             className={[
//               'pointer-events-auto flex items-start gap-3 rounded-xl border px-4 py-3 shadow-xl backdrop-blur',
//               t.kind === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : '',
//               t.kind === 'error' ? 'border-red-200 bg-red-50 text-red-800' : '',
//               t.kind === 'info' ? 'border-gray-200 bg-white/90 text-gray-800' : '',
//             ].join(' ')}
//           >
...
//       {/* Toolbar giống dashboard */}
//       <div className="flex flex-col items-start justify-between gap-3 p-4 bg-white border shadow-sm rounded-2xl md:flex-row md:items-center">
//         <div>
//           <h1 className="text-2xl font-bold tracking-tight">Quản lý sản phẩm</h1>
//           <p className="text-sm text-gray-600">Tạo/sửa sản phẩm, gallery & danh mục 2 bậc.</p>
//         </div>
//         <div className="flex items-center gap-2">
//           <button onClick={loadProducts} className="px-3 py-2 text-sm border rounded-xl hover:bg-gray-50">
//             Làm mới
//           </button>
//           <button onClick={startAdd} className="px-3 py-2 text-sm text-white bg-blue-600 rounded-xl hover:bg-blue-700">
//             + Thêm sản phẩm
//           </button>
//         </div>
//       </div>
...
//       {/* Table danh sách */}
//       {loading ? (
//         <div className="p-8 text-center text-gray-600 bg-white border rounded-2xl">Đang tải…</div>
//       ) : (
...
//                 <th className="px-3 py-3 text-xs font-semibold text-left text-gray-500 w-14">#</th>
//                 <th className="px-3 py-3 text-xs font-semibold text-left text-gray-500">Sản phẩm</th>
//                 <th className="px-3 py-3 text-xs font-semibold text-left text-gray-500 w-36">Slug</th>
//                 <th className="px-3 py-3 text-xs font-semibold text-left text-gray-500 w-28">SKU</th>
//                 <th className="w-32 px-3 py-3 text-xs font-semibold text-right text-gray-500">Giá</th>
//                 <th className="w-24 px-3 py-3 text-xs font-semibold text-left text-gray-500">Ảnh</th>
//                 <th className="w-20 px-3 py-3 text-xs font-semibold text-right text-gray-500">Tồn</th>
// <th className="w-24 px-3 py-3 text-xs font-semibold text-right text-gray-500">Đã bán</th>
// <th className="px-3 py-3 text-xs font-semibold text-left text-gray-500 w-28">Trạng thái</th>

//                 <th className="px-3 py-3 text-xs font-semibold text-right text-gray-500 w-44">Thao tác</th>
//               </tr>
//             </thead>
...
//                       {p.image ? (
//                         <img src={p.image} alt="" className="object-cover h-10 rounded w-14 ring-1 ring-gray-200" />
//                       ) : (
//                         <div className="h-10 bg-gray-100 rounded w-14" />
//                       )}
...
//                   </td>
//                   <td className="px-3 py-3">
//                     <span className="rounded bg-gray-100 px-2 py-0.5 text-xs">{(p as any).slug || ''}</span>
//                   </td>
//                   <td className="px-3 py-3">{p.sku || '—'}</td>
//                   <td className="px-3 py-3 text-right">{(p.price || 0).toLocaleString('vi-VN')}₫</td>
//                   <td className="px-3 py-3">{p.image ? '1 ảnh' : '—'}</td>
//                   <td className="px-3 py-3 text-right">{p.stock ?? 0}</td>
// <td className="px-3 py-3 text-right">{p.sold  ?? 0}</td>
// <td className="px-3 py-3">
//   {p.status === 'closed' ? (
...
//                       <button className="rounded-lg border px-2.5 py-1 text-xs hover:bg-gray-50" onClick={() => setViewing(p)}>
//                         Xem
//                       </button>
//                       <button className="rounded-lg border px-2.5 py-1 text-xs hover:bg-gray-50" onClick={() => startEdit(p)}>
//                         Sửa
//                       </button>
//                       <button
//                         className="rounded-lg border px-2.5 py-1 text-xs text-red-600 hover:bg-red-50 disabled:opacity-50"
...
//                           setEditing((prev) => ({ ...(prev as Product), name, slug: prev?.slug ? prev.slug : slugify(name) }));
//                         }}
//                       />
//                     </div>
...
//                           value={editing.slug}
//                           onChange={(e) => setEditing({ ...(editing as Product), slug: slugify(e.target.value) })}
//                         />
//                       </div>
//                       <div className="space-y-1.5">
...
//     onChange={(e) =>
//       setEditing({ ...(editing as Product), status: (e.target.value as 'open' | 'closed') })
//     }
//   >
...
//                 <Card title="Category" subtitle="Choose level 1; if 'Trang trí nhà' then add level 2.">
//                   {/* Bậc 1 */}
//                   <div className="mb-3">
//                     <div className="mb-2 text-xs font-medium text-gray-600">Product Category (Level 1)</div>
//                     <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
//                       {tops.map((t) => (
//                         <label key={t.id} className="flex items-center justify-between px-3 py-2 text-sm border rounded-lg hover:bg-slate-50">
...
//                         </label>
//                       ))}
//                       {!tops.length && <div className="px-3 py-2 text-xs text-gray-500 border rounded-lg">Không có danh mục</div>}
//                     </div>
//                   </div>
...
//                   {isTrangTriNha && (
//                     <div className="mb-1">
//                       <div className="mb-2 text-xs font-medium text-gray-600">Sub Categories (Level 2)</div>
//                       <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
//                         {subs.map((c) => (
//                           <label key={c.id} className="flex items-center justify-between px-3 py-2 text-sm border rounded-lg hover:bg-slate-50">
//                             <span className="truncate">
//                               {c.name} <span className="text-xs text-gray-500">/{c.slug}</span>
//                             </span>
//                             <input type="checkbox" className="w-4 h-4" checked={lv2.includes(c.id)} onChange={() => toggleLv2(c.id)} />
//                           </label>
//                         ))}
//                         {!subs.length && <div className="px-3 py-2 text-xs text-gray-500 border rounded-lg">Không có danh mục bậc 2</div>}
//                       </div>
//                     </div>
//                   )}
...
//                         className="w-full px-3 py-2 text-sm border rounded-lg hover:bg-slate-50"
//                       >
//                         Thêm vào gallery
//                       </button>
//                     </div>
...
//                         placeholder="0"
//                         value={editing.price}
//                         onChange={(e) => setEditing({ ...(editing as Product), price: Number(e.target.value) })}
//                       />
//                     </div>
//                     <div className="space-y-1.5">
...
//             {/* Footer actions: giống 3 nút ở ảnh */}
...
//                           <span className="inline-block w-3 h-3 rounded-full ring-1 ring-gray-300" style={{ background: c.hex || '#eee' }} />
//                           {c.name}
//                           {c.hex ? ` (${c.hex})` : ''}
//                         </span>
//                       ))
...
//   Tồn kho: <b>{viewing.stock ?? 0}</b> • Đã bán: <b>{viewing.sold ?? 0}</b>
// </div>
// <div className="text-sm text-gray-600">
