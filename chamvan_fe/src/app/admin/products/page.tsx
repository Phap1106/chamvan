...
// import { 
//   Search, Plus, X, Upload, ChevronUp, Trash2, 
//   Edit3, Eye, Image as ImageIcon, Package, Layers, Tag
// } from 'lucide-react';
...
//   s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/đ/g, 'd').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 120);
...
//   };
//   return <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-[11px] font-semibold border ${colors[color]}`}>{children}</span>;
// };
...
//   const [viewing, setViewing] = useState<Product | null>(null); // State cho chế độ Xem
...
//         let imagesRaw = p.images ?? [];
//         if (typeof imagesRaw === 'string') {
//            try { imagesRaw = JSON.parse(imagesRaw); } catch { imagesRaw = imagesRaw.split(','); }
//         }
//         const images = Array.isArray(imagesRaw) 
//           ? imagesRaw.map((i: any) => typeof i === 'string' ? i : i.url || i.src || '').filter(Boolean) 
//           : [];
...
//       const items = (Array.isArray(data) ? data : data.items || []).map((c:any) => ({ id: Number(c.id), name: c.name, slug: c.slug }));
//       setAllCats(items);
//     } catch {}
//   }
...
//       // Tự động điền gallery từ dữ liệu cũ
...
//     } else {
//       setEditing({ 
//         name: '', slug: '', price: 0, sku: '', description: '', 
//         image: '', images: [], stock: 0, sold: 0, status: 'open', colors: [], specs: [] 
//       });
//       setSelectedCatIds([]);
//       setGallery([]);
...
//       await loadProducts();
//       setShowDrawer(false);
//       toast.success(editing.id ? 'Cập nhật thành công' : 'Thêm mới thành công');
//     } catch (e) { toast.error('Lưu thất bại, kiểm tra lại dữ liệu'); } 
...
//       const matchQ = !q || p.name.toLowerCase().includes(lowerQ) || p.sku?.toLowerCase().includes(lowerQ);
//       const matchCat = catF === 'all' || p.categories?.some(c => c.id === catF);
...
//                     <td className="px-4 py-3"><Badge color={p.status === 'open' ? 'green' : 'zinc'}>{p.status === 'open' ? 'Active' : 'Draft'}</Badge></td>
...
//                  {/* eslint-disable-next-line @next/next/no-img-element */}
//                  <img src={viewing.image || '/placeholder.jpg'} className="w-full h-full object-cover" alt="" />
//               </div>
//               <div>
...
//               <section className="space-y-4">
//                 <div className="flex items-center gap-2 pb-2 border-b"><ImageIcon size={18}/><h3 className="font-bold">Hình ảnh</h3></div>
...
//               <section className="space-y-4">
//                  <div className="flex items-center gap-2 pb-2 border-b"><Layers size={18}/><h3 className="font-bold">Phân loại</h3></div>
//                  <div className="grid md:grid-cols-2 gap-6">
//                    <div className="space-y-2">
//                       <Label>Danh mục</Label>
//                       <div className="max-h-40 overflow-y-auto border border-zinc-300 rounded-md p-2 bg-white">
//                          {allCats.map(c => (
//                             <label key={c.id} className="flex items-center gap-2 p-1 hover:bg-zinc-50 cursor-pointer text-sm">
...
const slugify = (s: string) =>
  s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/đ/g, 'd').replace(/[^a-z0-9]+/g, '-').replace(/-{2,}/g, '-').slice(0, 120);
const formatVND = (n: number) => new Intl.NumberFormat('vi-VN').format(n);
const parseDigits = (s: string) => Number(s.replace(/[^\d]/g, '')) || 0;
