  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${API_BASE}/users/me/password`, {
        ...
        credentials: 'include', // gửi cookie cv_token sang BE
        body: JSON.stringify({
          currentPassword: cur,
          newPassword: n1,
        }),
      ...
      if (!res.ok) {
        ...
        // Gộp các dạng lỗi phổ biến
        const message =
          data?.message ||
          data?.error ||
          (typeof data === 'string' ? data : 'Đổi mật khẩu thất bại');
      ...
      setErrMsg(err?.message || 'Có lỗi xảy ra');
    } finally {
      setLoading(false);
    }
  }
    ...
    <AccountLayout title="Thay đổi mật khẩu">
      <form onSubmit={onSubmit} className="max-w-2xl">
        ...
        {errMsg && (
          <div
            role="alert"
            className="flex items-start gap-2 p-3 mb-5 text-sm border rounded-lg border-rose-200 bg-rose-50 text-rose-700"
          >
