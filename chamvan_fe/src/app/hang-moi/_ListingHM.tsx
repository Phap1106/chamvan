...
/* ===== Types UI (giữ nguyên logic, chỉ mở rộng type để thêm slug) ===== */
type UIProductCardWithSlug = UIProductCard & { slug?: string };
type BaseItem = UIProductCardWithSlug & { _createdAt?: string; _slugs: string[] };
...
function ListingHangMoiInner() {
  ...
  const { products, total } = useMemo(() => {
    ...
    const base: BaseItem[] = (state.data || []).map((p) => {
      const slugs = (p.categories ?? []).map((c) => c.slug);
      const img = normalizeImage(p.images ?? undefined, p.image ?? undefined);
      const colors = normalizeColors(p.colors);
    ...
    const pageItems: UIProductCardWithSlug[] = arr
      .slice(start, end)
      .map((p) => ({
        id: p.id,
        slug: p.slug,
