...
//   type: 'color' | 'image';
//   title: string;
//   frontLabel?: string; // text ngắn lúc chưa hover
//   subtitle?: string;
//   description?: string;
//   dimensions?: string;
//   placement?: string;
//   src?: StaticImageData;
//   align?: 'left' | 'right'; // chỉ dùng cho ô hồng
// };

// const TILE_LABEL = 'GỢI Ý NHANH';

// export default function TopPicks() {
//   // 7 ảnh minh hoạ – mô tả bao quát, không phụ thuộc ảnh cụ thể
...
//   // 2 ô màu hồng – front ngắn, inside chi tiết
...
//   // 9 tile hiển thị (7 ảnh + 2 ô hồng), ô hồng xen kẽ, không đứng cạnh nhau
...
//                   "
//                 >
//                   {/* mặt trước: chữ ngắn NỔI BẬT / SƯU TẬP */}
//                   <div
//                     className="
//                       absolute inset-0
...
//                     "
//                   >
//                     <span className="text-xl md:text-2xl font-extrabold tracking-[0.28em] uppercase">
//                       {item.frontLabel}
//                     </span>
//                   </div>

//                   {/* mặt trong: nội dung chi tiết, lật mở khi hover */}
//                   <div
//                     className={`
//                       absolute inset-0
...
//                     `}
//                   >
//                     <h3 className="text-2xl md:text-3xl font-bold tracking-[0.14em] uppercase">
//                       {item.title}
//                     </h3>
//                     {item.subtitle && (
//                       <p className="mt-4 text-xs md:text-sm font-semibold opacity-95 max-w-xs">
//                         {item.subtitle}
//                       </p>
//                     )}
//                     {item.description && (
//                       <p className="mt-3 text-xs md:text-sm opacity-90 max-w-md">
//                         {item.description}
//                       </p>
//                     )}
//                   </div>
...
//                     <p className="text-[10px] md:text-[11px] tracking-[0.18em] uppercase text-white/80">
//                       CHẠM VÂN • TOP PICK
//                     </p>
...
//                     <div className="mt-3 space-y-1.5 text-[11px] md:text-xs text-white/90">
//                       {item.dimensions && (
//                         <p>
...
function Tile({ item, idx, variant }: TileProps) {
  ...
  const paddingYColor = variant === 'desktop' ? '' : ''; // giữ mặc định
  ...
  if (isColor) {
    return (
      ...
      >
        ...
        {/* mặt trong: nội dung chi tiết */}
        <div
          className={`
            absolute inset-0
            flex flex-col justify-center
            ${paddingX} ${paddingYColor}
            opacity-0 translate-y-4
            transition-all duration-500
            group-hover:opacity-100 group-hover:translate-y-0
            ${alignRight ? 'items-end text-right' : 'items-start text-left'}
  ...
  return (
    ...
    >
      ...
      {/* overlay + text khi hover (desktop & mobile đều có, nhưng mobile ít khi hover) */}
      <div
        className="
          pointer-events-none
          absolute inset-0
    ...
    <section className="w-full pt-16 pb-16 md:pt-24 md:pb-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        ...
        <div className="text-center mb-10 md:mb-16">
          ...
          <h2 className="mt-3 text-2xl md:text-4xl font-semibold tracking-wide">
            SẢN PHẨM BÁN CHẠY
          </h2>
          <p className="mt-3 text-sm md:text-base text-zinc-500 max-w-2xl mx-auto">
            Một vài gợi ý giúp bạn hình dung nhanh kích thước, vị trí đặt và cách
        ...
        {/* MOBILE: thanh trượt ngang, chỉ 5 tile */}
        <div className="md:hidden -mx-4 px-4">
          <div
            ...
            "
          >
            {items.slice(0, MOBILE_MAX_ITEMS).map((item, idx) => (
              <Tile key={`m-${idx}`} item={item} idx={idx} variant="mobile" />
            ))}
          </div>
        </div>

        {/* DESKTOP/TABLET: grid 3 cột, đủ 9 tile */}
        <div className="hidden md:grid grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {items.map((item, idx) => (
            <Tile key={`d-${idx}`} item={item} idx={idx} variant="desktop" />
          ))}
        </div>
      </div>
    </section>
