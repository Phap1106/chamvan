...
//                 <div className="text-[16px] sm:text-[18px] text-neutral-400 line-through whitespace-nowrap">
//                   {priceRenderer(original)}
//                 </div>
//                 <div className="text-[16px] font-semibold text-red-600 whitespace-nowrap">
//                   -{discountPercent}%
//                 </div>
//               </div>
//             )}
  ...
  return (
    <Link href={href} className="block group" prefetch={false}>
      <div ref={cellRef} className="flex flex-col h-full">
        ...
        <div className="flex flex-col flex-1 min-w-0 pt-3">
          <h3 className="line-clamp-2 min-h-[52px] text-[17px] sm:text-[18px] font-medium text-neutral-900 leading-snug">
            {product.name}
          </h3>

          {/* ✅ FIX mobile overflow + đổi layout giá trên mobile */}
          <div className="flex flex-col min-w-0 gap-1 pt-2 mt-auto sm:flex-row sm:items-baseline sm:gap-2">
            {/* Original + % (mobile: nằm trên) */}
            {hasDiscount && (
              <div className="flex flex-wrap items-center order-1 min-w-0 sm:order-2 gap-x-2 gap-y-1">
                <div className="max-w-full text-[17px] sm:text-[16px] font-semibold text-neutral-500 line-through whitespace-nowrap">
                  {priceRenderer(original)}
                </div>
                <div className="shrink-0 text-[14px] font-semibold text-red-600 whitespace-nowrap">
                  -{discountPercent}%
                </div>
              </div>
            )}

            {/* Price (mobile: nằm dưới) */}
            <div
              className={[
                "order-2 sm:order-1 font-semibold leading-none whitespace-nowrap",
                hasDiscount ? "text-red-600" : "text-neutral-900",
                // mobile: nhỏ hơn giá gốc 1 chút; desktop giữ như cũ
                hasDiscount ? "text-[16px] sm:text-[17px]" : "text-[18px] sm:text-[19px]",
              ].join(" ")}
            >
              {priceRenderer(price)}
            </div>
