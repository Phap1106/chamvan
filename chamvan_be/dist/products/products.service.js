...
let ProductsService = class ProductsService {
    ...
    normPrice(v) { const n = Number(v); return Number.isFinite(n) ? n : 0; }
    async saveCoreAndChildren(input) {
        ...
        const categoryIds = (categoryIdsRaw || []).map((v) => Number(v)).filter((v) => Number.isFinite(v));
        const categories = categoryIds.length ? await this.cateRepo.find({ where: { id: (0, typeorm_2.In)(categoryIds) } }) : [];
    ...
    async getRecommendations(id, limit = 12) {
        ...
        const pool = [];
        const seen = new Set();
        [candidates1, candidates2, candidates3].forEach(list => list.forEach(p => { if (!seen.has(p.id)) {
            seen.add(p.id);
            pool.push(p);
        } }));
        const maxSold = Math.max(1, ...pool.map((p) => p.sold || 0));
        const related = pool.map((p) => {
